...
Can we say that countries with better health preparedness better perform in controlling the spread of COVID-19? Generally people might think this way. However, the United States, with the highest Global Health Security (GHS) Index[^GHSI], have reported the staggering number of cases over 15 million and over 300 thousand deaths from COVID-19, which is by far leading, as of December 12, 2020. Moreover, the wealthy countries do not seem to perform well, either. Not being against those claims, in this report, we attempt to explain the counter-intuitive pattern.
...
Blass and Penny note that COVID-19 is different then other natural disasters we have faced (2020).[^Blass] They criticize that the health service providers were not prepared for the novel coronavirus, unlike the previous epidemics. With the rise of online services, cybersecurity and protocols for distanced conferences and meetings have become vital.[^Blass] The column adds, staff members have difficulty working remotely, since they never had this experience before. They conclude that the global change and the characteristics of COVID-19 have changed the adequate guidelines to the possible future disasters, and the providers must prepare such procedures.
...
We chose three variables, the GHSI, healthcare expenditure, and the number of hospital beds, to briefly define the healthcare quality. We will look at the relationship between them and the dependent variable, COVID-19 associated deaths per million. Note that they are the official report from each country, and they do not reflect the actual, live-time counts. The data have been collected from this link: https://wzb-ipi.github.io/corona/. Control variables like population share of elders, regions will be considered as well, to discuss about the unexpected results.

We plotted the independent variables on the x-axis and the dependent variable on the y-axis, and we used linear regression to show the correlation. Then we added the confounding variables as colors, when needed, in order to display how they might contribute to the expected (or unexpected) results. 
...
Despite the more clear pattern for the countries with the lower-share, the pattern becomes very insignificant for the group of countries with the higher-share.
...
However, when we introduce regions, patterns appear: 
...
GGlobal Health Security (GHS) Index, created by Nuclear Threat Initiative, Johns Hopkins Center for Health Security, and The Economist Intelligence Unit, considers six measurements to determine each country’s healthcare preparedness: prevention, detection and reporting, rapid response, health system, compliance with international norms, and risk environment.[^GHSI] The initiative is to develop global healthcare readiness to prevent or effectively respond to disease epidemics.[^GHSI] We used the GHS index that was released in 2019.
...
As we tried to find the pattern from the complex data with unexpected results, we became more certain that we cannot find the correlation between healthcare preparedness and the actual death rate. Instead, it gives an idea to the relationship to the *reported* death. Better healthcare preparedness leads to better identification of patients; it leads to more confirmed cases; and that may lead to more reported deaths.
